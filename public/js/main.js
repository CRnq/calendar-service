...
function generateCalendar(year, month) {
  ...
  let dayCount = 1;
  let createYearMonthHtml =
    '<h1 class="year-month">' + year + "/" + month + "</h1>";
  ...
  for (let n = 0; n < 5; n++) {
    for (let d = 0; d < 7; d++) {
      ...
      if (n === 0 && d < firstDay) {
        createCalendarHtml +=
          '<div class="prev-month-day">' +
          (prevLastDayCount - (firstDay - d - 1)) +
          "</div>";
        ...
        isNextMonth = true;
        createCalendarHtml +=
          '<div class="next-month-day">' + (dayCount - lastDayCount) + "</div>";
        dayCount++;
      }
    }
  }
  ...
  tasks.forEach((task) => {
    ...
    const overlappingTasks = tasks.filter((t) => {
      ...
      const isWithinCurrentMonth =
        (taskStartMonth === currentMonth || taskEndMonth === currentMonth) &&
        taskStart.getDate() <= endDay &&
        taskEnd.getDate() >= startDay;
...
function generateWeekCalendar(year, month, date) {
  ...
  for (let d = 0; d < 7; d++) {
    createCalendarHtml +=
      '<div class="prev-month-day">' + (weekFirstDate + d) + "</div>";
  }
...
function generateDayCalendar(year, month, date) {
  ...
  let createYearMonthHtml =
    '<h1 class="year-month">' + year + "/" + month + "/" + todayDate + "</h1>";
  let createCalendarHtml = "";
  ...
  switch (date.getDay()) {
    case 0:
      createCalendarHtml +=
        '<div class="Sunday">' + weeks[date.getDay()] + "</div>";
      break;
    case 1:
    case 2:
    case 3:
    ...
    case 5:
      createCalendarHtml +=
        '<div class="weekday">' + weeks[date.getDay()] + "</div>";
      break;
    case 6:
      createCalendarHtml +=
        '<div class="Saturday">' + weeks[date.getDay()] + "</div>";
      break;
  }
